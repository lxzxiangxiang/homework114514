...
void Ball::applyEffect(EffectType et, const QList<Ball*>& allBalls)
{
    ...
    auto applyTimedEffect = [&](Ball* target) {
        for (auto& ae : target->m_effects) {
            if (ae.type == et) {
                ae.timer = qMax(ae.timer, timer);
                return;
            }
        }
        ActiveEffect ae;
        ae.type = et;
        ae.timer = timer;
        target->m_effects.append(ae);
    };

    auto applyGrowEffect = [&](Ball* target) {
        for (auto& ae : target->m_effects) {
            if (ae.type == EffectType::Grow) {
                ae.timer = qMax(ae.timer, timer);
                return;
            }
        }
        qreal originalMass = target->m_mass;
        qreal growFactor = GameConstants::Ball::Grow::RADIUS_MULTIPLIER * GameConstants::Ball::Grow::RADIUS_MULTIPLIER;
        target->m_mass *= growFactor;
        target->setMass(target->m_mass);
        ActiveEffect ae;
        ae.type = EffectType::Grow;
        ae.timer = timer;
        ae.growOriginalMass = originalMass;
        target->m_effects.append(ae);
    };

    if (et == EffectType::Grow) {
        applyGrowEffect(this);
    } else {
        applyTimedEffect(this);
    }
    ...
    for (Ball* other : allBalls) {
        ...
        if (et == EffectType::Grow) {
            applyGrowEffect(other);
        } else {
            applyTimedEffect(other);
        }
    }
}
...
void Ball::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget)
{
    ...
    if (hasEffect(EffectType::Shield)) {
        qreal shieldTimer = 0.0;
        for (const auto& ae : m_effects) {
            if (ae.type == EffectType::Shield) {
                shieldTimer = ae.timer;
                break;
            }
        }
